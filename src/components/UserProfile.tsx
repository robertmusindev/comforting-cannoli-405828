...
export const UserProfile = ({ user }: UserProfileProps) => {
  ...
  const [activeTab, setActiveTab] = useState<'stats' | 'shop' | 'inventory'>('stats');
                ...
                <div className="w-full md:w-80 lg:w-96 bg-sky-50 border-b-8 md:border-b-0 md:border-r-[6px] border-sky-200 p-6 flex flex-col items-center relative overflow-y-auto z-10" style={{ scrollbarWidth: 'none' }}>
                  ...
                  <div className="flex flex-col w-full gap-4 mt-auto">
                    ...
                    <button 
                      ...
                      className={`w-full py-5 rounded-3xl font-black text-lg uppercase transition-all flex items-center justify-center gap-3 border-[4px] shadow-sm transform ${
                        activeTab === 'shop' 
                        ? 'bg-amber-400 text-white border-amber-300 shadow-[0_6px_0_#d97706,inset_0_-4px_0_rgba(255,255,255,0.2)] translate-y-[2px]' 
                        : 'bg-white text-slate-500 border-slate-200 shadow-[0_6px_0_#cbd5e1] hover:bg-slate-50 hover:-translate-y-1'
                      }`}
                    >
                      <Coins size={24} className={activeTab === 'shop' ? 'text-white' : 'text-slate-400'} /> {t('shop') || 'NEGOZIO'}
                    </button>
                    <button 
                      ...
                      >
                        ...
                        <div className="bg-gradient-to-br from-amber-400 to-orange-500 p-8 rounded-[2rem] text-white mb-8 shadow-[0_8px_0_#b45309] relative overflow-hidden border-4 border-amber-300">
                          ...
                          <div className="relative z-10 text-center">
                            <p className="font-display font-black text-5xl md:text-6xl text-white uppercase leading-none drop-shadow-[0_4px_0_#9a3412] mb-2">NEGOZIO PB</p>
                              ...
                              >
                                ...
                                <button 
                                  onClick={() => {
                                    ...
                                    // Simulated in-app purchase flow
                                    addReward(0, false, bundle.pbAmount);
                                    
                                    // Delay celebration to ensure it starts AFTER the system alert/simulation overlay
                                    console.log('Scheduling celebration for:', bundle.name);
                                    setTimeout(() => {
                                    ...
                                    <div key={item.id} className="bg-white border-[4px] border-slate-100 rounded-[2rem] p-5 flex flex-col hover:border-indigo-100 transition-colors shadow-sm relative overflow-hidden">
                                      ...
                                      <button
                                        ...
                                        onClick={async () => {
                                          ...
                                          if (success) {
                                            audio.playCoinSound?.();
                                            // Trigger special hero animation if it's the Israel skin
                                            if (item.id === 'skin_special_israel') {
                                              setHeroSkin({ isOpen: true, skinId: item.id });
                                            }
                                            // Auto-equip skins on purchase
